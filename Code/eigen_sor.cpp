...
void sor(double dx, double dy, MatrixXd& u, MatrixXd& bound) {
	...
	*/
	...
	int iter = 1;
	int max_iter = 20000;
	int ysize = u.rows(), xsize = u.cols();
	double tol = 1e-6;		// The tolerance sets the digits precision of the solution
	double resid, rnorm, avg_norm, Ex, Ey;

  VectorXd x = VectorXd::LinSpaced(xsize, 0, xsize*dx);
  VectorXd y = VectorXd::LinSpaced(ysize, 0, ysize*dy);

  MatrixXd grad= MatrixXd::Zero(ysize,xsize);
