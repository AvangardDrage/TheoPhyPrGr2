...
void sor(int xsize, int ysize, double dx, double dy, vector< vector<double> > &u, vector< vector<double> > &bound) {
	...
	*/
	...
	vector< vector<double> > Ex(ysize, vector<double> (xsize, 0));
	vector< vector<double> > Ey(ysize, vector<double> (xsize, 0));
	...
	*/
	...
	for (i = 1; i < ysize-1; i++) {
		for (j = 1; j < xsize-1; j++) {
			...
			Ex[i][j] = -0.5*(u[i][j+1]-u[i][j-1])/dx;
			Ey[i][j] = -0.5*(u[i+1][j]-u[i-1][j])/dx;
	...
	for (i = 0; i < ysize; i+=10 ) {
		for (j = 0; j < xsize; j+=10) {
			outfile << x[j] << " " << y[i] << " " << u[i][j] << " " << grad[i][j] << " " << Ex[i][j] << " " << Ey[i][j] << endl;
		}
	}
